...
import { getKirchaListings, FrontendKirchaListing } from "@/lib/api/kircha";
...
function KirchaListContent() {
  ...
  const [listings, setListings] = useState<FrontendKirchaListing[]>([]);
  ...
  // Fetch live Kircha data from backend
  useEffect(() => {
    let isMounted = true;

    getKirchaListings({ category: typeParam as "ox" | "sheep" | "goat" })
      .then((data) => {
        if (isMounted) {
          setListings(data);
          setIsLoading(false);
      ...
      .catch((err) => {
        console.error("Error loading live Kircha data:", err);
        if (isMounted) {
          setIsLoading(false);
        }
      });
  ...
  return (
    <div className="bg-[#f2f4f2] min-h-screen pb-[140px] flex flex-col items-center">
      ...
      <div className="sticky top-0 z-40 w-full bg-white border-b border-[rgba(0,0,0,0.06)] shadow-2xs">
        {/* Top App Header */}
      ...
      <div className="w-full max-w-[430px] flex flex-col gap-[16px] items-center py-[6px] px-[14px]">
        {/* Loading Skeleton */}
        {isLoading ? (
          <div className="w-full flex flex-col gap-4 py-8 items-center">
            <div className="animate-spin rounded-full h-8 w-8 border-2 border-[#74a156] border-t-transparent" />
            <p className="text-xs text-stone-500 font-medium">
              {language === "am"
                ? "የቅርጫ መረጃዎችን በማምጣት ላይ..."
                : "Loading Kircha listings..."}
            </p>
          </div>
        ) : (
          ...
          <div className="flex flex-col gap-[16px] w-full items-center">
            {filteredListings.length > 0 ? (
                ...
                >
                  ...
                  <div className="p-[12px] pb-[16px] flex flex-col gap-[8px] w-full">
                    ...
                    <div className="flex items-center justify-between w-full">
                      <span className="text-[18px] font-bold text-[#1a1c19] leading-[26px]">
                        {listing.cattleName}
                      </span>
                      <span className="text-[11px] font-bold text-[#74a156] uppercase tracking-[0.6px] bg-[#74a156]/10 px-2 py-0.5 rounded-md">
                        {listing.tagNumber}
                      </span>
                    </div>
              ...
              <div className="bg-white rounded-[12px] p-8 text-center w-full border border-stone-200">
                <p className="text-[#62748e] text-[15px] font-medium">
                  {language === "am"
                    ? "ምንም የቅርጫ ዝርዝሮች አልተገኙም"
                    : "No active Kircha listings found"}
                </p>
              </div>
            )}
          </div>
